...
def buka_detail_data(pilihkost):
    with open('datakost.txt') as csv_file:
        ...
        for i in csv_reader:
            if i[0] == pilihkost:
                print(f"Nama Kost: {i[1]}, Jenis Kost: {i[2]}, Alamat: {i[3]}, Ukuran: {i[4]}, Fasilitas: {i[6]}, Narahubung: {i[7]}, Jumlah kamar yang tersedia: {i[8]}")
                return int(i[5])  # Mengembalikan harga kost
        return None
...
def main():
    ...
    if pilihan == "1":
        ...
        for kost in daftar_kost:
            if kost[0] == pilihkost:
                harga_kost = buka_detail_data(pilihkost)
                if harga_kost is not None:
                    jumlah_kamar = int(input("Masukkan jumlah kamar yang akan disewa: "))
                    total_harga = harga_kost * jumlah_kamar
